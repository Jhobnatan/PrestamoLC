...
const nuevo = (id) => new Promise((resolve, reject) => {
    let sueldo = 'SELECT `id_sueldo`, `sueldo`, `descripcion` FROM `jr_sueldo`';
    
    let grado = 'SELECT `id_grado`, `grado` FROM `jr_grado`';
...
const save_negocio =  (negocio) => new Promise((resolve, reject) => {
    ...
    let insertNegocio = "INSERT INTO `jr_negocio_general`(`razon_social`, `rnc`, `id_empleado`, `cant_sucursales`, `red_social`, `sap_acreedor`, `sap_deudor`, `estado`, `id_tipo_afiliacion`, `id_pago_electronico`)  VALUES ( ? );";
...
const negocio_general = (id) => new Promise((resolve, reject) => {
	let negocio_gral = 'SELECT `id_negocio`, `razon_social`, `rnc`, `id_empleado`, `cant_sucursales`, `red_social`, `sap_acreedor`, `sap_deudor`, `fecha_creacion`, `estado`, `id_tipo_afiliacion`, `id_pago_electronico` FROM `jr_negocio_general` WHERE `id_negocio` = ?';
...
const update_negocio =  (negocio,id_empleado) => new Promise( async (resolve, reject) => {
    ...
    const ng = await negocio_general(negocio[9])
    const pn = await productos_negocio(negocio[9])
    const json ={
        negocio_general:ng,
    ...
    /////////////////CREO EL INSERT DEL NEGOCIO////////////////////////////
    let updatetNegocio = "UPDATE `jr_negocio_general` SET `razon_social`=?,`rnc`=?,`cant_sucursales`=?,`red_social`=?,`sap_acreedor`=?,`sap_deudor`=?,`estado`=?,`id_tipo_afiliacion`=?,`id_pago_electronico`=? WHERE `id_negocio`= ?";
