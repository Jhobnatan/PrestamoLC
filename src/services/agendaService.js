...
const list = (id_empleado,id_departamento,accesos) => new Promise((resolve, reject) => {
	// Informacion del portador.
	let condicion ="";
	if(accesos.ver_citas)
	{
		condicion = "OR 1"
	}
	...
	let citas = "SELECT DISTINCT `id_cita`, `titulo`, `fecha`, `hora_inicio`, `hora_fin`, `color`, `motivo`, `estado`, `recordatorio`, `minuta`, `fecha_registro`, `registrada_por`, `lugar`, `geo`, `empresa`, `id_ejecutivo_interno` FROM `vista_cita_invitados` WHERE `id_ejecutivo_interno` = ? "+condicion+""
	//let citas = 'SELECT `id_cita`, `titulo`, `fecha`, `hora_inicio`, `hora_fin`, `color`, `motivo`, `estado`, `recordatorio`, `minuta`, `fecha_registro`, `registrada_por`, `lugar`, `geo`, `empresa` FROM `jr_cita`';
	let empresas = 'SELECT `id_negocio`, `razon_social`, `rnc`, `id_empleado`, `cant_sucursales`, `red_social`, `sap_acreedor`, `sap_deudor`, `fecha_creacion`, `estado`, `id_tipo_afiliacion`, `id_pago_electronico` FROM `jr_negocio_general`'
	Promise.all([
        localQuery(ejecutivointerno,),
        localQuery(ejecutivo_externo,),
        localQuery(ejecutivo_pago,),
		localQuery(citas,id_empleado),
		localQuery(empresas,),
    ]).then(data => {
        // console.log(data)
        resolve(data);
